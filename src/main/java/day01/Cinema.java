...
public class Cinema {
    ...
    public List<String> findMovieByTime(LocalDateTime time){
        ...
        for (Movie m : movieList){
            if (movieContainsTime(m, time)){
                result.add(m.getMovieTitle());
            }
        }
        return result;
    }

    private boolean movieContainsTime(Movie m, LocalDateTime time) {
        for (LocalDateTime actual :m.getStartTime()){
